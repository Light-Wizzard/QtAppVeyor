...
If ($env:PLATFORM -eq "x64" -And $env:MY_COMPILER -eq "Qt") {
    ...
    cmd /c cmake -G "MinGW Makefiles" -A x64 -DCMAKE_PREFIX_PATH="$env:CMAKE_PATH_PREFIX" -DCMAKE_INSTALL_PREFIX=AppDir -DCMAKE_C_COMPILER=/c/MinGW/bin/gcc.exe -DCMAKE_CXX_COMPILER=/c/MinGW/bin/g++.exe ..
    If ($?) {
